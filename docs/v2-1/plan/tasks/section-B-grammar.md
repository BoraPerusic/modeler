...
- [ ] **Add the `mappingProperty` rule and its sub-rules.** Place near the other property productions, after `searchBlockProperty` (~line 203). Order matters for readability — keep the top-level rule first, then helpers underneath:
  ...
  - `mappingColumnValue` collapses forms (b) and (c) from the design doc into one `object_` alternative — the semantic layer disambiguates by inspecting keys. ANTLR ambiguity is avoided.
...
- [ ] `git status` shows regenerated files under `packages/parser/src/generated/`, `packages/grammar/src/generated/`, and `packages/vscode-ext/syntaxes/`. Stage them with the `.g4` edit in one commit.
...
- `mappingColumnValue` deliberately collapses forms (b) (`{ target: <id> }`) and (c) (`{ target: { column: <id> } }`) into one `object_` alternative. ANTLR's longest-match would otherwise fight us; the semantic layer (Section D) is what gives them their distinct meanings.
